...
//GY helper function
  ...
  clearAll = () => {  //clear all todo items from this list
    const listName= this.props.match.params.listName;    
    fetch(
  ...
  clearComplete = () => { //clear completed todo items from this list
    const listName= this.props.match.params.listName;
    fetch(
      `http://localhost:5000/list/${listName}?completed=true`, 
      {method: 'DELETE'})
    .then(() => {
  ...
  count = () => { //count number of tood items not yet completed in this list //GY change name to countCompleted
    let total = 0;
  ...
  showAll = () => { //show all items regardless of completed status
    this.setState({filter: "ALL"});
  }

  showCompleted = () => { //show only completed items
    this.setState({filter: "COMPLETED"});
  }

  showActive = () => { //show only active items
    this.setState({filter: "ACTIVE"});
  }

  getVisibleTodos = () => { //return list of items based on filter    
    return this.state.todoList.filter( todo => { //GY change name to applyCompletedFilter - takes in todos, returns 
  ...
  setSearchTerm = (searchTerm) => { //sets the search term when input is changed
    this.setState({searchTerm});
  }

  searchResults = () => { //searches for searchterm  GY apply search filter
